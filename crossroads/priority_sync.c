...
void priority_lock_acquire(struct priority_lock* lock, int priority)
{
    ...
    if (lock->holder == thread_current()) {
        printf("WARNING: Thread already holds priority lock\n");
        return;
    }
...
bool priority_lock_try_acquire(struct priority_lock* lock, int priority)
{
    ...
    /* Check if current thread already holds this lock */
    if (lock->holder == thread_current()) {
        printf("WARNING: Thread already holds priority lock (try_acquire)\n");
...
void priority_lock_release(struct priority_lock* lock)
{
    ...
    /* Check if current thread actually holds this lock */
    if (lock->holder != thread_current()) {
        printf("WARNING: Releasing priority lock not held by current thread\n");
        return;
    }
